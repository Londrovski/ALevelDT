  ...
  <div class="jm-pdf-wrap">
    ...
    <div v-else-if="state === 'ready'" class="jm-pdf-stage">
      <div class="jm-pdf-info">
        ...
        <span class="jm-pdf-hint">Use arrows or keyboard ←→ to turn pages</span>
      </div>
...
async function init () {
  try {
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      pages.value.push(canvas.toDataURL('image/jpeg', 0.85))
      loadedPages.value = i
    }
    state.value = 'ready'
  } catch (e) {
...
.jm-pdf-info { display: flex; justify-content: center; align-items: center; gap: 20px; margin-bottom: 16px; }
...
.jm-spread {
  ...
  /* Fixed height so both pages match */
  height: 76vh;
}
.jm-spread-page {
...
.jm-spread-page--left  { box-shadow: inset -6px 0 16px rgba(0,0,0,0.15); }
.jm-spread-page--right { box-shadow: inset  6px 0 16px rgba(0,0,0,0.08); }
.jm-spread-spine {
  width: 8px; flex-shrink: 0;
  background: linear-gradient(to right, #888, #ddd 40%, #ddd 60%, #888);
  box-shadow: 0 0 10px rgba(0,0,0,0.4);
}
/* Image fills the full page area — no letterboxing */
.jm-spread-img {
  display: block;
...
.jm-pdf-controls { display: flex; justify-content: center; gap: 4px; padding: 14px 0 10px; }
